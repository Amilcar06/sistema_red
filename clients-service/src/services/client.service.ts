...
import ClientMetadata from '../models/client-metadata.model';
...
class ClientService {
  ...
  async updateMetadata(clientId: string, data: any) {
    // Verificar que el cliente existe en Postgres
    const cliente = await prisma.cliente.findUnique({
      where: { id: clientId },
    });
    ...
    // Actualizar o crear metadata en MongoDB
    return ClientMetadata.findOneAndUpdate(
      { clientId },
      {
        $set: {
  ...
  async getMetadata(clientId: string) {
    const metadata = await ClientMetadata.findOne({ clientId });
    return metadata || {};
  }
}
